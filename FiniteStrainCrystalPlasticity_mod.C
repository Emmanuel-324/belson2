...
FiniteStrainCrystalPlasticity::FiniteStrainCrystalPlasticity(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _lag_e_old(
        getMaterialPropertyOld<RankTwoTensor>(_base_name + "lage")), // Lagrangian strain of previous increment
    _gss(declareProperty<std::vector<Real>>(_base_name + "gss")),    // Slip system resistances
    _gss_old(getMaterialPropertyOld<std::vector<Real>>(
       _base_name +  "gss")),                                  // Slip system resistances of previous increment
    _acc_slip(declareProperty<Real>(_base_name + "acc_slip")), // Accumulated slip
    _acc_slip_old(
        getMaterialPropertyOld<Real>(_base_name + "acc_slip")), // Accumulated slip of previous increment
    _update_rot(declareProperty<RankTwoTensor>(
       _base_name +  "update_rot")), // Rotation tensor considering material rotation and crystal orientation
